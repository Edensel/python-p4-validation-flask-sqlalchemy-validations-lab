...
@listens_for(Author, 'before_insert')
def validate_unique_name(mapper, connection, target):
    if target.name:
        existing_author = Author.query.filter(Author.name == target.name).first()
        if existing_author and existing_author.id != target.id:
